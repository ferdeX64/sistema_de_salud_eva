...
import Atender from "@iconify-icons/ep/document-copy";
  ...
  <div class="main">
    <el-form ref="formRef" :inline="true" :model="form" class="search-form bg-bg_color w-[99/100] pl-8 pt-[12px]">
      <el-form-item label="Nombre:" prop="name">
        <el-autocomplete clearable  class="!w-[180px]" v-model="state" :fetch-suggestions="querySearchAsync" placeholder="Ingrese el nombre"
          @select="handleSelect">
        </el-autocomplete>
      </el-form-item>
      <el-form-item label="N° cédula:" prop="ncedula">
        <el-autocomplete clearable  class="!w-[180px]" v-model="cedula" :fetch-suggestions="querySearchAsyncCedulas" placeholder="Ingrese la cédula"
          @select="handleSelect">
        </el-autocomplete>
      </el-form-item>

      <el-form-item label="Sexo:" prop="sexo">
        <el-select v-model="form.sexo" placeholder="Seleccione el sexo" clearable class="!w-[180px]">
    ...
    <PureTableBar title="Historias clínicas" :columns="columns" @refresh="onSearch">
      <template #buttons>
        ...
        <pure-table align-whole="center" showOverflowTooltip table-layout="auto" :loading="loading" :size="size" adaptive
          ...
          <template #operation="{ row }">
            <el-button class="reset-margin" link type="success" :size="size" :icon="useRenderIcon(Atender)"
              @click="DialogCreatePaciente('Agregar paciente', row)">
              Ver historia
            </el-button>
